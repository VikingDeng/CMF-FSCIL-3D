...
from .utils import *
from data.path import ShapenetTrainPath, ShapenetTestPath
from data.map import main_category_map, all_category_map
...
class ShapeNetTrain(data.Dataset):

    def __init__(self, task_id, task_type="all"):
        path = ShapenetTrainPath
        ...
        self.main_pc_path = path.PC_All

        self.index_file = path.Index_File
        self.npoints = config.npoints
        self.cache_dir = "data/shapenet/cache/train"
        self.check_dir = "log/check"
        self.main_category_map = all_category_map if self.task_id == 0 else main_category_map
        self.all_category_map = all_category_map

        # self.tokenizer = AutoTokenizer.from_pretrained("models/tokenizer/gpt2", use_fast=True, local_files_only=True)
        ...
        # self.cache_file = (
        #     os.path.join(self.cache_dir, f"task_{self.task_id}.pkl")
        #     if task_id != -2
        #     else os.path.join(self.cache_dir, f"task_0.pkl")
        # )
        # if self.task_id == -1:
        #     (
        #         self.main_data_list,
        ...
        #     else:
        #         print(
        #             f"Loading shapenet-train task {self.task_id} data from original source..."
        #         )
        #         if self.task_id == 0:
        #             (
        #                 self.main_data_list,
        ...
        #             ) = self.load_data_form_base()
        #         else:
        #             self.main_data_list, self.main_label_list = (
        #                 self.load_data_for_increment()
        #             )
    ...
    def check_main_all(self, main_file_name, all_file_name):
        if os.path.splitext(main_file_name)[1] != os.path.splitext(all_file_name)[1]:
            raise FileCheckError(
                f"Extension mismatch: {main_file_name} vs {all_file_name}"
            )
        ...
        if not all_base.startswith(main_base):
            raise FileCheckError(
                f"Base name mismatch 3: {main_file_name} vs {all_file_name}"
            )
    ...
    def check_pc_img(self, pc_name, img_name):
        ...
        if not pc_angle_match:
            ...
            if pc_base_name != img_base_name:
                raise FileCheckError(f"Base name mismatch 1: {pc_name} vs {img_name}")
            ...
            if pc_base_name != img_base_name:
                raise FileCheckError(f"Base name mismatch 2: {pc_name} vs {img_name}")
    ...
    def process_category(self, cate):
        try:
            ...
            main_pc_files = sorted(
                f
                for f in os.listdir(os.path.join(self.main_pc_path, cate))
                if f.endswith(".npy")
            )
            main_img_files = sorted(
                f
                for f in os.listdir(os.path.join(self.main_img_path, cate))
                if f.endswith("_r_000.png")
            )

            assert len(main_pc_files) == len(
                main_img_files
            ), f"Mismatch in number of files for category {cate}, for pc: {len(main_pc_files)}, for img: {len(main_img_files)}"
            ...
            for sub in sub_dirs:
                try:
                    ...
                    all_pc_file = sorted(
                        f for f in os.listdir(sub_pc_dir) if f.endswith(".npy")
                    )
                    all_img_file = sorted(
                        f for f in os.listdir(sub_img_dir) if f.endswith(".png")
                    )
                    ...
                    for i in range(num_groups):
                        ...
                        selected_pc_file = next(
                            (
                                f
                                for f in all_pc_file[start_idx:end_idx]
                                if f.endswith(f"_r{angle}.npy")
                            ),
                            None,
                        )
                        selected_img_file = next(
                            (
                                f
                                for f in all_img_file[start_idx:end_idx]
                                if f.endswith(f"_r_{angle}.png")
                            ),
                            None,
                        )
                    ...
                    assert len(selected_pc_files) == len(
                        selected_img_files
                    ), f"Mismatch in selected files for category {cate}, subdir {sub}, pc: {len(selected_pc_files)}, img: {len(selected_img_files)}"
                    ...
                    print(
                        f"File check failed in subdirectory {sub} for category {cate}: {e}"
                    )
                except Exception as e:
                    print(
                        f"Unexpected error in subdirectory {sub} for category {cate}: {e}"
                    )
            ...
            for i in tqdm(range(len(main_pc_files)), desc=f"Loading train {cate} data"):
                try:
                    ...
                    for j, sub in enumerate(sub_dirs):
                        ...
                        all_data["pc"].append(
                            os.path.join(sub_pc_dir, all_pc_files[j][i])
                        )
                        all_data["img"].append(
                            os.path.join(sub_img_dir, all_img_files[j][i])
                        )
    ...
    def load_data_form_base(self):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {
                executor.submit(self.process_category, cate): cate for cate in cates
            }
    ...
    def load_data_for_increment(self):
        task_id = 0 if self.task_id == -2 else self.task_id
        with open(
            os.path.join(self.index_file, f"train/task_{task_id}.txt")
        ) as f:
            paths = f.readlines()
        main_label_list = []
        main_data_list = []
        for file_path in paths:
            ...
            main_label_list.append(self.main_category_map[category])
    ...
    def load_prompt(self):
        ...
        for main_cate in os.listdir(self.main_text_path):
            main_prompt_path = os.path.join(
                self.main_text_path, main_cate, "prompt.txt"
            )
            with open(main_prompt_path, "r") as f:
                main_prompt = f.readline()

            tokenized_caption = [self.tokenizer(main_prompt)]
            res[self.main_category_map[main_cate]] = torch.stack(tokenized_caption)
            ...
            for sub in os.listdir(all_prompt_dir):
                ...
                res[self.all_category_map[f"{main_cate}_{sub}"]] = torch.stack(
                    tokenized_caption
                )
        return res
    ...
    def check_seq(self):
        ...
        output_file_path = os.path.join(
            self.check_dir, f"check_seq_shuffle_{current_time}.txt"
        )
        ...
        if self.task_id == 0 or self.task_id == -1:
            if idx % 5 == 0:
                ...
                return (
                    pc_data,
                    img_data,
                    self.prompt_dic[self.main_label_list[int(idx / 5)]],
                    self.main_label_list[int(idx / 5)],
                )
            else:
                pc_data = self.process_pc(
                    self.all_data_list[int(idx / 5)]["pc"][(idx % 5) - 1]
                )
                img_data = self.process_img(
                    self.all_data_list[int(idx / 5)]["img"][(idx % 5) - 1]
                )
                return (
                    pc_data,
                    img_data,
                    self.prompt_dic[self.all_label_list[int(idx / 5)][(idx % 5) - 1]],
                    self.all_label_list[int(idx / 5)][(idx % 5) - 1],
                )
        elif self.task_id == -2:
            pc_data = self.process_pc(self.main_data_list[int(idx)])
    ...
    def __len__(self):
        return (
            5 * len(self.main_data_list)
            if self.task_id == 0
            else len(self.main_data_list)
        )
...
class ShapeNetTest(data.Dataset):
    ...
    def __init__(self, task_id):
        ...
        self.list_of_points,self.list_of_labels = self.load_data_from_source()
        ...
        #             cache_file = os.path.join(self.caload_data_from_sourceche_dir, f"task_{i}.pkl")
        ...
        #     else:
        #         print(
        #             f"Loading shapenet-test task {self.task_id} data from original source..."
        #         )
        #         pc, label = self.load_data_from_source()
        #         self.list_of_points.extend(pc)
        #         self.list_of_labels.extend(label)
    ...
    def load_data_from_source(self):
        for i in range(self.task_id+1):
            with open(self.indexfile + f"/test/task_{self.task_id}.txt", "r") as f:
                self.datapath.extend(f.readlines())
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            list(
                tqdm(
                    executor.map(process_data, range(len(self.datapath))),
    ...
    def check_seq(self):
        ...
        # 将时间戳添加到文件名中
        output_file_path = os.path.join(
            self.check_dir, f"shapenet_test_check_seq_{current_time}.txt"
        )
